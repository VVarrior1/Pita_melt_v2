...
export default function AdminOrderCard({
  ...
  const getTimeInfo = () => {
    if (!isMounted) {
      return { label: "", value: "", color: "text-gray-400", urgent: false };
    }
    const pickup = new Date(order.estimatedPickupTime);
    const diffSeconds = Math.round((pickup.getTime() - now.getTime()) / 1000);
    if (diffSeconds <= 0) {
      return {
        label: "Overdue by",
        value: formatDuration(diffSeconds),
        color: "text-red-400",
        urgent: true,
      };
    }
  ...
  return (
    <div
      className={`bg-gray-900 rounded-lg border border-gray-800 shadow-sm ${
        timeInfo.urgent ? "ring-1 ring-yellow-400/40" : ""
      }`}
    >
      <div className="p-6">
        {/* Header */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">
                Order for {order.customerInfo.name || "Guest"}
              </h3>
              <p className="text-sm text-gray-400">
                Pickup at {formatTime(order.estimatedPickupTime)}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="text-right">
                <div className={`font-semibold ${timeInfo.color}`}>
                  ...
                  <div className="mt-2 space-y-1">
                    {Object.entries(item.customizations).map(([key, value]) => (
                      <div key={key} className="text-sm text-gray-300">
                        <span className="text-[#f17105] capitalize">
                          {key}:
                        </span>{" "}
